...
export default function DashboardUtilizationTab({ sohList, locations }: Props) {
    ...
    locations.forEach((l: any) => {
        const locGroup = (l.location_group || '').trim();
        if (!locGroup) return;
        
        if (!monthlyTableMap[locGroup]) {
            monthlyTableMap[locGroup] = {
                key: locGroup,
            ...
            months.forEach(m => {
                monthlyTableMap[locGroup][`${m}_occupiedCount`] = 0;
            });
        }
        monthlyTableMap[locGroup].capacity++;
        
        const locName = (l.location || '').trim();
        if (locName) {
            months.forEach(m => {
                if (monthOccupiedLocs[m].has(locName)) {
                    monthlyTableMap[locGroup][`${m}_occupiedCount`]++;
                }
            });
        }
    });

    const finalMonthlyRows = Object.values(monthlyTableMap).sort((a, b) => a.locationGroup.localeCompare(b.locationGroup));
    ...
    finalMonthlyRows.forEach(row => {
        gmCapacity += row.capacity;
        months.forEach(m => {
            const occCount = row[`${m}_occupiedCount`];
            const pctOcc = row.capacity > 0 ? ((occCount / row.capacity) * 100).toFixed(1) : '0.0';
            row[`${m}_pctOccupied`] = pctOcc;
            gmMonthlyOccupied[m] += occCount;
        });
    });
    ...
    months.forEach(m => {
        ...
        grandRow[`${m}_pctOccupied`] = gmCapacity > 0 ? ((gmMonthlyOccupied[m] / gmCapacity) * 100).toFixed(1) : '0.0';
    });
    finalMonthlyRows.push(grandRow);

    const monthlyColumns: any[] = [
        { title: 'location_group', dataIndex: 'locationGroup', key: 'locationGroup', width: 180, render: (v: string, r: any) => r.isSummary ? <span style={{ fontWeight: 700, color: '#fff' }}>{v}</span> : v, fixed: 'left' },
        { title: 'capacity', dataIndex: 'capacity', key: 'capacity', width: 80, render: (v: number, r: any) => r.isSummary ? <span style={{ fontWeight: 700, color: '#fff' }}>{v}</span> : v, fixed: 'left' },
    ];
    ...
    months.forEach(m => {
        monthlyColumns.push({
            ...
            children: [
                {
                    title: 'Occupied',
                    dataIndex: `${m}_occupiedCount`,
                    key: `${m}_occupiedCount`,
                    width: 100,
                    align: 'center',
                    render: (v: number, r: any) => r.isSummary ? <span style={{ fontWeight: 700, color: '#60a5fa' }}>{v}</span> : <span style={{ color: '#60a5fa' }}>{v}</span>
                },
                {
                    title: '%Occupied',
                    ...
                    width: 100,
                    align: 'center',
                    render: (v: string, r: any) => r.isSummary ? <span style={{ fontWeight: 700, color: '#4ade80' }}>{v}%</span> : <span style={{ color: '#4ade80' }}>{v}%</span>
                }
            ]
        });
    });
