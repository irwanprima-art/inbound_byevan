...
import {
    ...
    Card, Tabs, Popconfirm, Modal, Divider, Statistic, Row, Col, Popover, Badge,
} from 'antd';
import {
...
<table class="page-table">
    ...
    const filteredDocs = docs.filter(d => {
        ...
        return searchTerms.some(q =>
            (d.doc_number || '').toLowerCase().includes(q) || (d.doc_type || '').toLowerCase().includes(q) || (d.checker || '').toLowerCase().includes(q)
        );
    });
                                        ...
                                        {soLoading ? (
                                            ...
                                            <>
                                                <Row gutter={16} style={{ marginBottom: 16 }}>
                                                    ...
                                                    <Col span={4}><Card size="small" style={{ background: soSummary.accuracy >= 100 ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)', border: 'none', textAlign: 'center' }}><Statistic title={<span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11 }}>Accuracy</span>} value={soSummary.accuracy.toFixed(3)} suffix="%" valueStyle={{ color: soSummary.accuracy >= 100 ? '#10b981' : '#f87171', fontSize: 20 }} /></Card></Col>
                                        ...
                                        <div style={{ marginBottom: 12 }}>
                                            ...
                                            <div className="summary-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                                                ...
                                                <div className="summary-box" style={{ border: '1px solid #ccc', borderRadius: 6, padding: 10, textAlign: 'center' }}>
                                                    ...
                                                    <div className="value" style={{ fontSize: 20, fontWeight: 700, color: s.accuracy >= 100 ? '#16a34a' : '#dc2626' }}>{s.accuracy.toFixed(3)}%</div>
                                                </div>
                                            </div>
